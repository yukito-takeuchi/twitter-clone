...
export default function BookmarksPage() {
  ...
  const fetchBookmarks = async () => {
    ...
    try {
      ...
      const fetchedPosts = await bookmarkApi.getBookmarkedPosts(user.id, LIMIT, 0);
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      ...
      const morePosts = await bookmarkApi.getBookmarkedPosts(user.id, LIMIT, offset);
      ...
      {loading ? (
        ...
        <Box sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 1 }}>
            気に入った投稿をブックマークして、後で簡単に見つけられるようにしましょう。
          </Typography>
        </Box>
      ) : (
        <Box>
